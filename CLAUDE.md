...
- **Node 下限**:≥ 20
- **关键约定**:能力文件 schema 严格 8 字段(`name`/`description`/`init`/`execute`/`destroy`/`deps`/`externalDeps`/`provide`),多一个就拒
- **v1.1 新增**:`provide` 字段 + `process.minlo.call('<name>.<fn>', ...args)` —— 能力 A export `provide: { fn1, fn2 }`,能力 B 用 `call` 调用,见 [docs/design.md §3.12](docs/design.md#312-跨能力-api-调用provide-字段--processminlocall)。`call` 目标必须在 `deps` 里(拓扑序)
...
| 区分"设计"和"现状" | [docs/design.md](docs/design.md) = 设计定稿(v1.0);[docs/STATUS.md](docs/STATUS.md) = 现在能做到什么 |
...
- 用户提"加个能力"→ 写到 `.minlo/abilities/<name>.js`(.ts 也行),改 mission JSON,跑 `minlo list` 验证
- 用户提"能力间互相调用"→ 用 `provide` 字段 + `process.minlo.call('<name>.<fn>', ...)` —— **不**在 `process.minlo.ctx` 里手挂对象
