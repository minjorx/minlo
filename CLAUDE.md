...
- **Node 下限**:≥ 22(`process:minlo` 虚拟模块需要 `module.register()`)
- **关键约定**:能力文件 schema 严格 7 字段(`name`/`description`/`init`/`execute`/`destroy`/`deps`/`externalDeps`),多一个就拒
- **v1.1 新增**:`process:minlo` 虚拟模块 — 能力可 `provide` / `use` 互相调用,见 [docs/design.md §3.12](docs/design.md#312-跨能力-api-直调processminlo-虚拟模块)。用了 `process:minlo` 的能力**必须**是 `.js`,且 `use` 目标必须在 `deps` 里
...
| 能力间怎么互相调用(provide / use) | [docs/design.md §3.12](docs/design.md#312-跨能力-api-直调processminlo-虚拟模块) |
...
# 测试
npm test                               # node --test test/ (含 process:minlo 6 个用例)
```

## 占位规则(给 Claude 自己)

- 用户提"加个能力"→ 写到 `.minlo/abilities/<name>.js`(TS 也可,但若用 `process:minlo` 必须 `.js`),然后改 mission JSON,跑 `minlo list` 验证
- 用户提"能力间互相调用"→ 用 `process:minlo` 的 `provide` / `use`,**不**在 `process.minlo.ctx` 里手挂对象
