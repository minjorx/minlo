...
> 版本: v1.0 · 状态: 设计定稿
>
> 完整面向使用者的介绍见 [`README.md`](../README.md)。贡献者流程见 [`CONTRIBUTING.md`](../CONTRIBUTING.md)。

## 目录

1. [项目概述](#1-项目概述)
   - [1.1 什么是 Minlo？](#11-什么是-minlo)
   - [1.2 核心设计理念](#12-核心设计理念)
   - [1.3 技术选型](#13-技术选型)
2. [目录结构规范](#2-目录结构规范)
   - [2.1 用户项目布局](#21-用户项目布局)
   - [2.2 目录职责矩阵](#22-目录职责矩阵)
3. [核心概念定义](#3-核心概念定义)
   - [3.1 能力（Ability）](#31-能力ability) — 8 字段 schema、加载校验
   - [3.2 生命周期与执行方法](#32-生命周期与执行方法) — init / execute / destroy 时机
   - [3.3 任务（Mission）](#33-任务mission) — JSON 编排格式
   - [3.4 （已移除）会话](#34-已移除会话)
   - [3.5 工作区（Workspace）](#35-工作区workspace)
   - [3.6 全局能力（Global Abilities）](#36-全局能力global-abilities)
   - [3.7 能力间依赖（deps）](#37-能力间依赖deps)
   - [3.8 跨能力通信（process.minlo.ctx）](#38-跨能力通信processminloctx)
   - [3.9 能力配置（process.minlo.configs）](#39-能力配置processminloconfigs)
   - [3.10 外部依赖（externalDeps）](#310-外部依赖externaldeps)
   - [3.11 例子：llm 能力](#311-例子llm-能力openai-兼容-chat-completion)
   - [3.12 跨能力 API 调用（provide / call）](#312-跨能力-api-调用provide-字段--processminlocall)
4. [执行生命周期与顺序界定](#4-执行生命周期与顺序界定)
   - [4.1 阶段一：初始化](#41-阶段一初始化执行一次)
   - [4.2 阶段二：主循环（execute 返回值协议）](#42-阶段二主循环v1-死循环无-maxiterations)
   - [4.3 阶段三：清理](#43-阶段三清理执行一次)
   - [4.4 顺序界定规则总结](#44-顺序界定规则总结)
5. [能力发现与启动期注册](#5-能力发现与启动期注册)
   - [5.1 扫描范围](#51-扫描范围启动期一次性)
   - [5.2 注册表结构（AbilityRecord）](#52-注册表结构)
   - [5.3 启动期加载顺序（严格 schema 校验）](#53-启动期加载顺序含严格-schema-校验)
   - [5.4 destroy 调用责任](#54-destroy-调用责任)
   - [5.5 TypeScript 能力文件](#55-typescript-能力文件)
   - [5.6 不监听运行中变更](#56-不监听运行中变更)
6. [CLI 命令规范](#6-cli-命令规范)
   - [6.1 全局命令](#61-全局命令)
   - [6.2 任务管理（minlo mission）](#62-任务管理minlo-mission)
7. [配置优先级](#7-配置优先级)
...
| 严格 schema | 能力文件只能导出 `name` / `description` / `init` / `execute` / `destroy` / `deps` / `externalDeps` 七项；多出任何字段即拒绝该文件 |
...
| 热插拔机制 | 启动期 `import()` 重新扫描，主循环期间锁定 | 无需外部工具，dev 体验最简 |
...
| `init` | function | ❌（三选一至少一个） | 生命周期钩子：能力加载时调用一次 |
...
**`execute(params)` 是一切"做事"的方法**。框架怎么调它、调几次、传什么 `params`，由框架与智能体共同决定。能力作者**不需要关心**"我是 tool 还是 hook"——这是 v1 框架不再区分的语义。
...
// 2. 字段白名单检查：除 name/description/init/execute/destroy 外，只允许 deps/externalDeps
...
- 能力间数据传递走 `process.minlo.ctx`（详见 §3.8）
   ...
   minlo: missing external dependencies:
     ...
     - openai (required by "llm" — global ability)
          ...
          (This installs into the project's node_modules, which is required
           even for global abilities — see §3.10.)
   ```
5. exit code 1
...
minlo: cannot import llm.js (global ability): missing npm package "openai"
       ...
       Install it in your project's node_modules:
         npm install openai
       (This is required even for global abilities — see §3.10.)
```
...
- **global 能力同样必须在项目根安装依赖**——minlo **不**从自身 `node_modules` 解析 global 能力的 deps（v1 简化：统一从项目根解析）。这是 §10 全局默认能力（如 `llm`）的一个已知 UX 权衡：使用全局能力前先在项目根 `npm install <deps>`
...
| 4 | 创建 `process.minlo.ctx = {}` 与 `process.minlo.configs = {}` | 跨能力通信的共享对象（详见 §3.8 / §3.9） |
...
| `minlo run <name:default>` | `<name>` 优先；不存在则 `default` | 两者都找不到时报错退出 |
| `minlo run <name:x>` | `<name>` 优先；不存在则 `<x>` | 同上，`x` 是任意任务名（不一定叫 default） |
...
| 追求类型安全 / 团队协作 | 迁移到 TypeScript | 1. `npm install -D tsx @types/node`<br>2. 将 `.js` 改为 `.ts`<br>3. 添加 `tsconfig.json` |
